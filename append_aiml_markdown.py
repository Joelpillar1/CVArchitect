...
for i, (theme, kw_str, scope, result) in enumerate(themes_data):
    ...
    md = f"**Role:** {role} | **Theme:** {theme} | **Seniority:** {seniority} | **Verb Context:** systems\n"
    md += f"**Keywords:** {kw_str}\n\n"
    md += f"**Scope:** {scope}\n"
    md += f"**Result:** {result}\n\n"
    md += f"**Variations:**\n"
    md += f"- **A (Standard):** {var_a}\n"
    md += f"- **B (Result-first):** {var_b}\n"
    md += f"- **C (Impact-led):** {var_c}\n"
    md += f"- **D (Concise):** {var_d}\n\n"
    md += f"**Variables:**\n"
    md += f"  - `{{countA}}`: 3 to 20, step 1 (integer)\n"
    md += f"  - `{{countB}}`: 10 to 500, step 10 (integer)\n"
    md += f"  - `{{metricA}}`: 15 to 40, step 5 (percentage)\n"
    md += f"  - `{{metricB}}`: 10 to 30, step 5 (percentage)\n"
...
print(f"Added {added_count} brand NEW bullets.")
