...
export default function StyledTemplate({ data }: StyledTemplateProps) {
  ...
  const accentColor = data.accentColor || '#374151';
  ...
  const renderSectionHeader = (title: string) => (
    <div className="w-full py-1 px-3 mt-0 mb-2 bg-gray-100 break-inside-avoid rounded-sm box-border">
      <h2
        className={`section-header tracking-widest font-bold leading-tight ${getSectionHeaderAlignment()}`}
        style={{
  ...
  const renderSkillsBlock = (title: string, skillsStr: string) => {
    ...
    return (
      <section className="break-inside-avoid" style={{ marginBottom: sectionGap }}>
        ...
        <ul
          data-skills-grid
          className={`list-none pl-0 grid gap-x-8 gap-y-1 text-gray-700 ${data.skillsColumnCount === 2 ? 'grid-cols-2' : (data.skillsColumnCount === 4 ? 'grid-cols-4' : 'grid-cols-3')}`}
          style={bodyStyle}
        >
