...
export default function DevTemplate({ data }: { data: ResumeData }) {
  ...
  const renderSectionHeader = (title: string) => (
    <div className={`flex items-center gap-2 mb-2 break-inside-avoid ${getSectionHeaderAlignment()}`}>
      <span className="w-1 h-4 rounded shrink-0" style={{ backgroundColor: accentColor }}></span>
      <h2
        className="section-header font-bold"
          ...
          if (data.location || data.address) {
            contactItems.push(
              <span key="loc" className="flex items-center gap-1">
                {showIcons && <MapPin size={12} color={accentColor} />}
          ...
          if (data.phone) {
            contactItems.push(
              <span key="ph" className="flex items-center gap-1">
                {showIcons && <Phone size={12} color={accentColor} />}
          ...
          if (data.email) {
            contactItems.push(
              <span key="em" className="flex items-center gap-1">
                {showIcons && <Mail size={12} color={accentColor} />}
          ...
          if (data.linkedin) {
            contactItems.push(
              <span key="li" className="flex items-center gap-1">
                {showIcons && <Linkedin size={12} color={accentColor} />}
                <a
                  data-path="linkedin"
          ...
          if (data.atHandle) {
            contactItems.push(
              <span key="at" className="flex items-center gap-1">
                {showIcons && <Send size={12} color={accentColor} />}
