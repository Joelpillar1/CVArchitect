...
const StudentTemplate: React.FC<StudentTemplateProps> = ({ data }) => {
  ...
  const bodySize = fontSizes?.body || 9;
  const bodyStyle = {
  ...
  const getJobTitleAlignClass = () => {
    if (data.jobTitleAlignment === 'left') return 'text-left';
    if (data.jobTitleAlignment === 'right') return 'text-right';
    if (data.jobTitleAlignment === 'center') return 'text-center';
    if (data.headerAlignment === 'left') return 'text-left';
    if (data.headerAlignment === 'right') return 'text-right';
    return 'text-center';
  };

  const getContactAlignClass = () => {
    if (data.contactAlignment === 'left') return 'justify-start text-left';
    if (data.contactAlignment === 'right') return 'justify-end text-right';
    if (data.contactAlignment === 'center') return 'justify-center text-center';
    if (data.headerAlignment === 'left') return 'justify-start text-left';
    if (data.headerAlignment === 'right') return 'justify-end text-right';
    return 'justify-center text-center';
  };

  const renderSectionHeader = (title: string) => (
    <div className={`section-header-wrap break-inside-avoid w-full mb-2 mt-2 ${getSectionHeaderAlignment()}`}>
      <h2
  ...
  const renderSkillsBlock = (title: string, skillsStr: string) => {
    ...
    return (
      <section className="break-inside-avoid" style={{ marginBottom: `${getSectionGapIn(data)}in` }}>
        ...
        <ul
          data-skills-grid
          className={`list-none pl-2.5 grid gap-x-8 gap-y-1 text-gray-800 ${data.skillsColumnCount === 2 ? 'grid-cols-2' : (data.skillsColumnCount === 4 ? 'grid-cols-4' : 'grid-cols-3')}`}
          style={bodyStyle}
        >
  ...
  const renderBulletsBlock = (title: string, content: string[] | string, basePath: string = 'keyAchievements') => {
    ...
    return (
      <section className="break-inside-avoid" style={{ marginBottom: `${getSectionGapIn(data)}in` }}>
        {renderSectionHeader(title)}
        <ul className="list-disc list-outside ml-5 space-y-1 text-gray-800 text-justify" style={bodyStyle}>
          ...
          return (
            ...
            >
              <div className="flex justify-between items-baseline mb-0.5">
                <div>
                  <h3 className="font-bold text-gray-900" style={bodyStyle}>
                    {exp.role && (
                ...
                {dateRange && (
                  <span className="text-gray-600 font-medium italic shrink-0 ml-4" style={bodyStyle}>
          ...
          return (
            ...
            >
              <div className="flex justify-between items-baseline">
                ...
                {edu.year && (
                  <span data-path={`${basePath}.${index}.year`} className="text-gray-600 font-medium shrink-0 ml-4">
        ...
        <div className="space-y-1.5 text-gray-800" style={bodyStyle}>
          {validItems.map((cert, index) => (
            <div key={cert.id} className="flex justify-between items-baseline">
              ...
              {cert.date && (
                <span data-path={`${basePath}.${index}.date`} className="text-gray-600 font-medium shrink-0 ml-4">
    ...
    switch (resolved.rendererKind) {
      ...
      case 'coursework':
        return renderCourseworkBlockHelper({
          ...
          renderSectionHeader,
          renderTextBlock,
          bodyStyle: { fontSize: `${bodySize}pt`, lineHeight: data.lineHeight || 1.5 },
        });
      case 'custom':
    ...
    >
      ...
      >
        ...
        {isTitleFirst(data, false) && data.jobTitle && (
          <p
            data-path="jobTitle"
            className={`tracking-wider font-semibold text-gray-700 ${getJobTitleAlignClass()}`}
            style={{
        ...
        {(() => {
          ...
          return (
            <div
              className={`flex flex-wrap items-center ${showIcons ? 'gap-x-3 gap-y-1' : 'gap-x-1.5 gap-y-1'} text-gray-600 ${getContactAlignClass()}`}
              style={{
                ...bodyStyle,
