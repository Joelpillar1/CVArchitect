...
export default function SageTemplate({ data }: { data: ResumeData }) {
  ...
  const renderSectionHeader = (title: string) => (
    ...
    >
      <h2
        className={`section-header font-bold tracking-wide leading-tight ${getSectionHeaderAlignment()}`}
        style={{
          ...
          if (data.location || data.address) {
            contactItems.push(
              <span key="loc" className="inline-flex items-center gap-1">
                {showIcons && <MapPin size={12} className="shrink-0" color={ACCENT} />}
          ...
          if (data.phone) {
            contactItems.push(
              <span key="ph" className="inline-flex items-center gap-1">
                {showIcons && <Phone size={12} className="shrink-0" color={ACCENT} />}
          ...
          if (data.email) {
            contactItems.push(
              <span key="em" className="inline-flex items-center gap-1">
                {showIcons && <Mail size={12} className="shrink-0" color={ACCENT} />}
                <a
                  data-path="email"
          ...
          if (data.linkedin) {
            contactItems.push(
              <span key="li" className="inline-flex items-center gap-1">
                {showIcons && <Linkedin size={12} className="shrink-0" color={ACCENT} />}
                <a
                  data-path="linkedin"
          ...
          if (data.atHandle) {
            contactItems.push(
              <span key="at" className="inline-flex items-center gap-1">
                {showIcons && <Send size={12} className="shrink-0" color={ACCENT} />}
