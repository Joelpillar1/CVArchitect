...
export default function EliteTemplate({ data }: { data: ResumeData }) {
  ...
  const renderSectionHeader = (title: string) => (
    <h2
      className={`section-header font-bold tracking-wide mb-1.5 flex items-center gap-2 ${getSectionHeaderAlignment()}`}
  ...
  const renderTextBlock = (title: string, text: string, path: string = 'summary') => {
    ...
    return (
      <section className="break-inside-avoid" style={{ marginBottom: `${getSectionGapIn(data)}in` }}>
        ...
        <div data-path={path} className="text-gray-700 whitespace-pre-line" style={bodyStyle}>
          <RichText text={text ?? ''} />
        </div>
      </section>
    );
