...
export default function ElegantTemplate({ data }: ElegantTemplateProps) {
  ...
  const accentColor = data.accentColor || '#1a237e'; // Navy blue
  ...
  const bodyAlignment = data.bodyHeaderAlignment || data.sectionHeaderAlignment || 'center';

  const renderSectionHeader = (title: string) => (
    <div className="flex items-center gap-4 mb-2 mt-0 break-inside-avoid">
      {(bodyAlignment === 'center' || bodyAlignment === 'right') && (
        <div className="flex-1 border-t border-dashed border-gray-400"></div>
      )}
      <h2
        className="section-header tracking-normal font-bold whitespace-nowrap leading-tight"
        style={{
